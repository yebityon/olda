...
namespace olda{
    ...
    bool is_exist(const std::vector<std::string>v, std::string s){
        for(auto & u : v)
            if(s.find(u) != std::string::npos) return true;
        return false;
    }
    ...
    analyze_omni_log(FileDatas fd){
        ...
        for(std::string log :  omni_graph.omni_log){
            ...
            if ( is_exist(entry_orders, log) ){
                ...
                handle_method_entry(log,omni_graph);
                
            } else if( is_exist(method_param,log) ) {
                
                handle_method_param( log,omni_graph);
                
            } else if ( is_exist(exit_orders, log) ){
                
                handle_method_exit(log,omni_graph);    
                
            } else if ( is_exist(write_orders, log)) {
                
                auto tmp = omni_graph.object_order;
                if(log.find("ARRAY") != std::string::npos){
                    handle_write_array(log,omni_graph);    
                } else {
                    handle_write_order(log,omni_graph);
                }
            
            } else if ( is_exist(read_orders, log )){
                
            } else {
                if(omni_graph.vertex_stack.empty()) continue;
